...
@dispatch
def realize(core, schema: LineageSeed, encode, path=()):
    """Identity realize — LineageSeed derivation happens at process
    construction (``realize_link``), not here. State always stores the
    base seed; the derived value is computed once just before passing
    config to ``__init__``. Keeps save/load round-trips invariant of
    the current ``DerivationContext`` and avoids the double-application
    that would happen from ``core.realize``'s discover+final passes.
    """
    if encode is None:
        encode = default(schema)
    outschema, outstate, merges = realize(
        core, schema._value, encode, path=path)
    schema._value = outschema
    return schema, outstate, merges
