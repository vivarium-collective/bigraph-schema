...
@dispatch
def bind(schema: Empty, state, to, to_schema, to_state, context):
    return schema, None


@dispatch
def bind(schema: Maybe, state, to, to_schema, to_state, context):
    if state is None:
        return Empty(), state
    else:
        return bind(schema._value, state, to, to_schema, to_state, context)


@dispatch
def bind(schema: Wrap, state, to, to_schema, to_state, context):
    return bind(schema._value, state, to, to_schema, to_state, context)


@dispatch
def bind(schema: Union, state, to, to_schema, to_state, context):
    for option in schema._options:
        if check(option, state):
            return bind(option, state, to, to_schema, to_state, context)
    return Empty(), None


@dispatch
def bind(schema: Tuple, state, to: Key, to_schema, to_state, context):
